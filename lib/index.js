...
exports.init = function (sequelize, optionsArg) {
  ...
  var log = options.log || console.log; // show the current sequelize and options objects
  ...
  _.extend(sequelize.Model, {
    hasPaperTrail: function hasPaperTrail() {
      ...
      this.addHook('afterDestroy', createAfterHook('destroy')); // create association
  ...
  function createBeforeHook(operation) {
    var beforeHook = function beforeHook(instance, opt) {
      ...
      previousVersion = _.omitBy(previousVersion, function (i) {
        return i != null && _typeof(i) === 'object' && !(i instanceof Date);
      });
      previousVersion = _.omit(previousVersion, options.exclude);
      currentVersion = _.omitBy(currentVersion, function (i) {
        return i != null && _typeof(i) === 'object' && !(i instanceof Date);
      });
      currentVersion = _.omit(currentVersion, options.exclude); // Disallow change of revision
  ...
  function createAfterHook(operation) {
    var afterHook = function afterHook(instance, opt) {
      if (debug) {
        ...
        if (ns) {
          log("CLS ".concat(options.continuationKey, ":"), ns.get(options.continuationKey));
        }
      }
      ...
      if (instance.context && (instance.context.delta && instance.context.delta.length > 0 || destroyOperation)) {
        ...
        if (options.enableRevisionChangeModel) {
          var _RevisionChange = sequelize.model(options.revisionChangeModel);
        }
  ...
  return {
    ...
    defineModels: function defineModels(db) {
      ...
      if (options.enableRevisionChangeModel) {
        ...
        var _RevisionChange2 = sequelize.define(options.revisionChangeModel, attributes, {
          underscored: options.underscored
        }); // Set associations


        Revision.hasMany(_RevisionChange2, {
          foreignKey: options.defaultAttributes.revisionId,
          constraints: false
        });

        _RevisionChange2.belongsTo(Revision);

        if (db) {
          db[_RevisionChange2.name] = _RevisionChange2;
        }
      }
